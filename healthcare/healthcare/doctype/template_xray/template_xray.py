...
class TemplateXray(Document):
	def before_insert(self):
		if self.link_existing_item and self.item:
			...
			if price_list:
				self.xray_rate = price_list[0].get("price_list_rate")
	...
	def validate(self):
		if (
			...
			and (not self.xray_rate or self.xray_rate <= 0.0)
		):
	...
	def update_item(self):
		...
		if item:
			item.update(
				{
					"item_name": self.xray_name,
					"item_group": self.xray_group,
					"disabled": 0,
					"standard_rate": self.xray_rate,
					"description": self.xray_description,
				}
			)
	...
	def validate_conversion_factor(self):
		if self.template_xray_type == "Single" and self.secondary_uom and not self.conversion_factor:
			frappe.throw(_("Conversion Factor is mandatory"))
		if self.template_xray_type == "Compound":
		...
		if self.template_xray_type == "Grouped":
			for group in self.xray_groups:
				if (
...
def create_item_from_template(doc):
	...
	item = frappe.get_doc(
		{
			"doctype": "Item",
			"item_code": doc.xray_code,
			"item_name": doc.xray_name,
			"item_group": doc.xray_group,
			"description": doc.xray_description,
			"is_sales_item": 1,
			"is_service_item": 1,
			"is_purchase_item": 0,
			"is_stock_item": 0,
	...
	# Insert item price
	if doc.is_billable and doc.xray_rate != 0.0:
		price_list_name = frappe.db.get_value(
			"Selling Settings", None, "selling_price_list"
		) or frappe.db.get_value("Price List", {"selling": 1})
		if doc.xray_rate:
			make_item_price(item.name, doc.xray_rate)
		else:
			make_item_price(item.name, 0.0)
	# Set item in the template
	frappe.db.set_value("Template Xray", doc.name, "item", item.name)

	doc.reload()


@frappe.whitelist()
def change_test_code_from_template(xray_code, doc):
	doc = frappe._dict(json.loads(doc))
	if frappe.db.exists({"doctype": "Item", "item_code": xray_code}):
		frappe.throw(_("Xray Item {0} already exist").format(xray_code))
	else:
		rename_doc("Item", doc.item, xray_code, ignore_permissions=True)
		frappe.db.set_value(
			"Template Xray", doc.name, {"xray_code": xray_code, "xray_name": xray_code}
		)
		rename_doc("Template Xray", doc.name, xray_code, ignore_permissions=True)
	return xray_code
